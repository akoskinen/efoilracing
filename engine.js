...
function completeLap() {
    ...
    const avgSpeedKmh = (sumSpeeds / frameCount) * speedConversion;
    ...
    document.getElementById('lapTimeDisplay').innerText = `Laptime: ${lapTimeStr}`;
    
    // Add lap to history
    const historyDiv = document.getElementById('lapHistory');
    const lapEntry = document.createElement('div');
    lapEntry.innerHTML = `Lap: ${lapTimeStr}s | Top: ${topSpeedKmh.toFixed(1)} | Avg: ${avgSpeedKmh.toFixed(1)} | Min: ${minSpeedKmh.toFixed(1)} km/h${collidedThisLap ? ' (+10s penalty)' : ''}`;
    ...
    if (!collidedThisLap && currentLapTime > 10 && distanceTraveled > 100) {
        ...
        // (the showInputForm function we modified now checks if it's a top 15 time)
        // Adding a slight delay to ensure telemetry updates are visible
        setTimeout(() => {
...
function drawTelemetry() {
  ...
  y += 20;
    ...
    y += 18;
            if (lap.collided) {
                ctx.fillText(`  Time:   ${lap.finalTime.toFixed(2)} s  (Disqualified: collision, +10s penalty!)`, x, y);
            } else {
...
function updateGhostStats() {
    ...
    const avgSpeed = currentGhost.frames[0].avgSpeedKmh;
...
function updateLapTimeDisplay() {
    ...
    const penaltyText = collidedThisLap ? ' (+10s penalty!)' : '';
